...
namespace HunterAI.Scripts
{
	public class CompanionMovement : MonoBehaviour
	{
		...
		CustomFiniteStateMachine<CompanionMovement> stateMachine;
		...
		{
			return Vector3.Distance(this.transform.position, this._player.position);
		}
		...
		public GameObject GetClosestEnemy()
		{
			...
			{
				float distanceToPlayer = Vector3.Distance(enemy.transform.position, this._player.position);
		...
		public GameObject GetClosestArrow()
		{
			...
			{
				float distanceToPlayer = Vector3.Distance(arrow.transform.position, this._player.position);
				if (distanceToPlayer < minimumDistance)
				{
					minimumDistance = distanceToPlayer;
					targetArrow = arrow;
		...
		void Start()
		{
			...
			this._player = GameObject.FindGameObjectsWithTag("Player")[0].transform;

			this.stateMachine = new CustomFiniteStateMachine<CompanionMovement>(this);
